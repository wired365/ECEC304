...
;-----------------------------------------------
export  VR_PGA_Start
export _VR_PGA_Start
export  VR_PGA_SetPower
export _VR_PGA_SetPower

export  VR_PGA_SetGain
export _VR_PGA_SetGain

export  VR_PGA_Stop
export _VR_PGA_Stop

IF (VR_PGA_AGNDBUFAPI)
export  VR_PGA_EnableAGNDBuffer
export _VR_PGA_EnableAGNDBuffer
export  VR_PGA_DisableAGNDBuffer
export _VR_PGA_DisableAGNDBuffer
ENDIF
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: VR_PGA_Start
;  FUNCTION NAME: VR_PGA_SetPower
;
;  DESCRIPTION:
...
;
   ...
   mov  A, reg[VR_PGA_GAIN_CR2]                            ; read power value
   and  A, ~POWERMASK                                ; clear power bits in A
   or   A, [X]                                       ; combine power value with balance of reg.
   mov  reg[VR_PGA_GAIN_CR2], A                       ; move complete value back to register
   pop  A
...
;
   ...
   mov  A, reg[VR_PGA_GAIN_CR0]                            ; read mux settings
   and  A, ~GAINREGMASK                                   ; clear gain bits in A
   tst  [X],HIGHGAIN                                      ; See if High Gain is set
   jnz  .SETHIGHGAIN
   and  reg[VR_PGA_GAIN_CR3],~EXGAIN                       ; Clear High Gain bit.
   or   A, [X]                                            ; combine gain value with balance of reg.
   mov  reg[VR_PGA_GAIN_CR0], A                            ; move complete value back to register
   pop  A
   RAM_EPILOGUE RAM_USE_CLASS_2
   ret
...
.SETHIGHGAIN:
   ...
   or   A, [X]                                            ; combine gain value with balance of reg.
   mov  reg[VR_PGA_GAIN_CR0], A                            ; move complete value back to register
   or   reg[VR_PGA_GAIN_CR3], EXGAIN                       ; Set High Gain bit.
   pop  A
   RAM_EPILOGUE RAM_USE_CLASS_2
   ret
.ENDSECTION
...
_VR_PGA_Stop:
   ...
   and REG[VR_PGA_GAIN_CR2], ~POWERMASK
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: VR_PGA_EnableAGNDBuffer
;
;  DESCRIPTION:
...
;    functions.
;
 VR_PGA_EnableAGNDBuffer:
_VR_PGA_EnableAGNDBuffer:
   RAM_PROLOGUE RAM_USE_CLASS_1
   and   reg[VR_PGA_GAIN_CR3], ~AGNDBUFMASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: VR_PGA_DisableAGNDBuffer
;
;  DESCRIPTION:
...
;    functions.
;
 VR_PGA_DisableAGNDBuffer:
_VR_PGA_DisableAGNDBuffer:
   RAM_PROLOGUE RAM_USE_CLASS_1
   or    reg[VR_PGA_GAIN_CR3], AGNDBUFMASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
ENDIF

; End of File VR_PGA.asm
